...
import static com.google.blockly.android.Constants.colorMap;
import static com.google.blockly.android.Constants.color_ids;
                    ...
                    case XmlPullParser.START_TAG:
                        if (parser.getName().equalsIgnoreCase("category")) {
                            ...
                            for(int i=0; i<category.getSubcategories().size(); i++) {
                                BlocklyCategory item = category.getSubcategories().get(i);
                                for(int j=0; j<item.getItems().size(); j++) {
                                    Block block = ((BlocklyCategory.BlockItem) (item.getItems().get(j))).getBlock();
                                    colorMap.put(block.getType(), color_ids[i%color_ids.length]);
                                    Log.v("BlocklyCategory", "i = " + i);
                                            Log.v("BlocklyCategory", "block.getType() = " +block.getType());
                                    Log.v("BlocklyCategory", "color_ids[index%color_ids.length] = " +color_ids[i%color_ids.length]);
                                }
                            }
...
//                            blockItem.getBlock().setColor(color_ids[index%color_ids.length]);
