...
public class QRResultActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imageMap.put("蓝牙模块",R.mipmap.bg_bluetooth);
        ...
        hintMap.put("红外循迹模块","检测原理是红外发射管发射光线到路面，红外光遇到白底则被反射，接收管接收到反射光，经施密特触发器整形后输出低电平；当红外光遇到黑线时则被吸收，接收管没有接收到反射光，经施密特触发器整形后输出高电平");
        imageMap.put("红外避障模块",R.mipmap.bg_tracking);
        hintMap.put("红外避障模块","检测原理是发射管发射红外线后，遇到障碍物后红外线反射回来被接收管接收，接收管导通，经比较器电路处理后，模块输出低电平，如果红外线没有遇到障碍物就不会被反射回，接收管不导通，经比较器电路处理后，模块输出高电平。");
        ...
        imageMap.put("电源驱动板",R.mipmap.bg_power_source);
        hintMap.put("电源驱动板","电机驱动板将电池电源与电机以及Arduino开发板连接器，使电池成为Arduino开发板提供电源，Arduino开发板控制电机");
